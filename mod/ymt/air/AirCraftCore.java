...
public class AirCraftCore extends NekonoteCore {
	...
	public Set<Integer> getDefaultMoveableSet() {
		...
		for (int i = 0; i < blockops.length; i++) { // target Ç™ñ¢éwíËÇÃéûÇ…ÇÕ defaultMoveableSet
			if (blockops[i] != null) {
				result.add(i);
			}
		}
	...
	public Set<Integer> getMoveableBlockIds() {
		...
		// ÇªÇÃëº
		result.remove(0); // ãÛãCÉuÉçÉbÉNÇÕà⁄ìÆã÷é~
		result.remove(Block.bedrock.blockID); // ä‚î’ÇÕà⁄ìÆã÷é~
	...
	public BlockData toSafeClientBlock(BlockData data) {
		// TODO renderStrategy ÉIÉvÉVÉáÉìÇí«â¡
		// 2 = ïœä∑ÇµÇ»Ç¢
		// 1 = Operator ÇÃñ≥Ç¢ÉuÉçÉbÉNÇïœä∑
		// 0 = Operator ÇÃñ≥Ç¢ BlockContainer Çïœä∑
		// -1 = èÌÇ…órñ—Ç…ïœä∑
		if (getBlockOperator(data.block.blockID) instanceof NullOperator == false) {
			// Operator Ç™ë∂ç›Ç∑ÇÈÇ»ÇÁà¿ëS
			return data;
		}
		if (data.block instanceof BlockContainer == false) {
			// BlockContainer Ç≈Ç»ÇØÇÍÇŒÇΩÇ‘ÇÒà¿ëS
			return data;
		}
		// à¿ëSÇ≈Ç»Ç¢Ç‡ÇÃÇÕ Block.cloth Ç…ïœä∑
	...
	protected void init() {
		if (0 < blockIdPyxis) {
			// ÉuÉçÉbÉNìoò^
			int[] texture = {
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_bottom.png"),
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_side.png"),
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_top0.png"),
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_top1.png"),
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_top2.png"),
				ModLoader.addOverride("/terrain.png", "/mod/ymt/air/py_top3.png"),
			};
			blockPyxis = new BlockPyxis(blockIdPyxis, texture).setBlockName("Pyxis");
			ModLoader.registerBlock(blockPyxis);
			Utils.addName(blockPyxis, "Pyxis", "óÖêjî’");
			...
			ModLoader.addEntityTracker(baseMod, EntityPyxis.class, entityIdPyxis, 256, 4, true); // ï`é ãóó£Ç∆ÇËÇ†Ç¶Ç∏çLÇﬂÇ…Ç∆ÇÈ
